...
public class GeminiAdapter implements IaRecommendationPort {
    ...
    public IaResponse getRecommendation(String imageUrl) {
        ...
        try {
            ...
            if (imageBytes == null || imageBytes.length == 0) {
                ...
                throw new ExternalServiceException("Error: No se pudo descargar la imagen de la URL: " + imageUrl);
            }

            String base64 = Base64.getEncoder().encodeToString(imageBytes);


            log.info("Base64 generado (Tamaño: {} bytes). Inicio: {}...", imageBytes.length, base64.substring(0, Math.min(base64.length(), 50)));
                    ...
                    .onStatus(status -> status.isError(), response ->
                            response.bodyToMono(String.class)
                                    ...
                                    .flatMap(errorBody -> {
                                        ...
                                        log.error("Error {} de Gemini. Detalle del cuerpo: {}", response.statusCode().value(), errorBody);
                                        return reactor.core.publisher.Mono.error(new ExternalServiceException(
                                                "Error HTTP de Gemini: " + response.statusCode().value() + " - Detalle: " + errorBody, e
                                        ));
                                    })
                    )
            ...
            try {
                ...
                throw new ExternalServiceException("Gemini devolvió un formato JSON inesperado o inválido: " + rawJson, e);
            }
            ...
            throw new ExternalServiceException("Fallo de conexion o procesamiento con el servicio de IA", e);
        }
    }
}